...
{		$wpgpxmaps_googlemapsv3_apikey = get_option('wpgpxmaps_googlemapsv3_apikey');
    ...
    wp_enqueue_script( 'highcharts', "//code.highcharts.com/highcharts.js", array('jquery'), "5.0.14", true);
...
function handle_WP_GPX_Maps_Shortcodes($attr, $content='')
{
	...
	$gpxy =                findValue($attr, "gpx",                "",                          		 "");
  $gpx =                parse_url($gpxy, PHP_URL_PATH);
...
// Folgende Zeilen hinzugefügt
	...
	if ($download=='true' && $gpxurl != '') {
		...
		$output.="<a class='button btn' href='$gpxurl' target='_new' download>".__("Last ned GPX-data", "wp-gpx-maps")."</a>";
	}
